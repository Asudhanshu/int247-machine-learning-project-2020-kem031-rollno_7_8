...
content = Image.open("content10.jpg").convert("RGB")
content = transform(content).to(device)
print("Content shape => ", content.shape)
style = Image.open("style10.jpg").convert("RGB")
...
print("device = ",device) 
#since graphics driver not available if available set to cuda
...
for i in range(1,epochs+1):
    ...
    if i%print_after == 0:
        ...
        plt.show()
        img_array = imcnvt(target)
        img_array = (img_array-np.min(img_array))/(np.max(img_array)-np.min(img_array))
        #plt.imsave(str(i)+'.png',imcnvt(target),format='png')
        plt.imsave(str(i)+'.png',img_array,format = 'PNG')
